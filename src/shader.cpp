...
#include "vignette_red.frag.xxd"
#include "vignette_green.frag.xxd"
...
GradientSpriteShader::GradientSpriteShader()
{
  INIT_SHADER(simple, gradient_sprite, GradientSpriteShader);
  ShaderBase::init();
  GET_U(origin);
  GET_U(resolution);
  GET_U(c1);
...
void GradientSpriteShader::set_origin(const Vec2 &origin)
{
  gl.Uniform2f(u_origin, origin.x, origin.y);
}
...
GrayVignetteShader::GrayVignetteShader()
{
  INIT_SHADER(simple, vignette_gray, GrayVignetteShader);
  ShaderBase::init();
  GET_U(resolution);
}
...
RedVignetteShader::RedVignetteShader()
{
  INIT_SHADER(simple, vignette_red, RedVignetteShader);
  ShaderBase::init();
  GET_U(resolution);
}

GreenVignetteShader::GreenVignetteShader()
{
  INIT_SHADER(simple, vignette_green, GreenVignetteShader);
  ShaderBase::init();
  GET_U(resolution);
}

BlueVignetteShader::BlueVignetteShader()
{
  INIT_SHADER(simple, vignette_blue, BlueVignetteShader);
  ShaderBase::init();
  GET_U(resolution);
}

YellowVignetteShader::YellowVignetteShader()
{
  INIT_SHADER(simple, vignette_yellow, YellowVignetteShader);
  ShaderBase::init();
  GET_U(resolution);
}

BlackVignetteShader::BlackVignetteShader()
{
...
{
  INIT_SHADER(simple_rect, polygon, PolygonShader);
  ShaderBase::init();
  GET_U(center);
  GET_U(rect_wh);
  GET_U(color);
...
void PolygonShader::set_center(const Vec2 &center)
{
  gl.Uniform2f(u_center, center.x, center.y);
}
...
void PolygonShader::set_sides(float sides)
{
  gl.Uniform1f(u_sides, sides);
}
...
void PolygonShader::set_angle(float angle)
{
  gl.Uniform1f(u_angle, angle);
}
