...
public:
  ...
  void bind_vignette_shader(VignetteShader &shader)
  {
    shader.bind();
    shader.set_resolution(w, h);
    apply_shader(shader);
  ...
  void composite_vignette()
  {
    apply_scissors();
    VignetteShader &shader = shState->shaders().vignette;
    bind_vignette_shader(shader);
  }

  void composite_gray_vignette()
  {
    apply_scissors();
    GrayVignetteShader &shader = shState->shaders().vignette_gray;
    bind_vignette_shader(shader);
  }

  void composite_sepia_vignette()
  {
    apply_scissors();
  ...
  void composite_red_vignette()
  {
    apply_scissors();
    RedVignetteShader &shader = shState->shaders().vignette_red;
    bind_vignette_shader(shader);
  }

  void composite_green_vignette()
  {
    apply_scissors();
    GreenVignetteShader &shader = shState->shaders().vignette_green;
    bind_vignette_shader(shader);
  }

  void composite_blue_vignette()
  {
    apply_scissors();
    BlueVignetteShader &shader = shState->shaders().vignette_blue;
    bind_vignette_shader(shader);
  }

  void composite_yellow_vignette()
  {
    apply_scissors();
    YellowVignetteShader &shader = shState->shaders().vignette_yellow;
    bind_vignette_shader(shader);
  }

  void composite_black_vignette()
  {
    apply_scissors();
    BlackVignetteShader &shader = shState->shaders().vignette_black;
    bind_vignette_shader(shader);
  }
...
struct GraphicsPrivate
{
  ...
  void composite_to_vignette_buffer(TEXFBO &buffer, int mode)
  {
    if (mode == 0)
      screen.composite_vignette();
    else if (mode == 1)
      screen.composite_gray_vignette();
    else if (mode == 2)
      screen.composite_sepia_vignette();
    else if (mode == 3)
      screen.composite_red_vignette();
    else if (mode == 4)
      screen.composite_green_vignette();
    else if (mode == 5)
      screen.composite_blue_vignette();
    else if (mode == 6)
      screen.composite_yellow_vignette();
    else if (mode == 7)
      screen.composite_black_vignette();
    set_buffer(buffer);
  }
...
Bitmap *Graphics::snap_to_vignette(int mode)
{
  Bitmap *bitmap = new Bitmap(width(), height());
  p->composite_to_vignette_buffer(bitmap->getGLTypes(), mode);
  return bitmap;
}
...
void Graphics::playMovie(const char *filename, int volume, bool res_win, bool skippable)
{
  ...
  if (movie->preparePlayback()) {
    int vid_w = (int)movie->video->width;
    int vid_h = (int)movie->video->height;
    int limiterDisabled = p->fpsLimiter.disabled;
    p->fpsLimiter.disabled = false;
    int old_fps = getFrameRate();
    ...
    clip_sprite.setBitmap(movie->videoBitmap);
    Debug() << get_fullscreen();
    if (res_win && !get_fullscreen()) {
      if (old_w != vid_w || height() != vid_h) {
        resizeScreen(vid_w, vid_h, true);
        res_change = true;
      }
    } else {
      double ratio = std::min((double)old_w / vid_w, (double)old_h / vid_h);
      clip_sprite.setZoomX(ratio);
      clip_sprite.setZoomY(ratio);
      clip_sprite.setX((old_w / 2) - (vid_w * ratio / 2));
      clip_sprite.setY((old_h / 2) - (vid_h * ratio / 2));
    }
    Sprite letterboxSprite;
    Bitmap letterbox(old_w, old_h);
    letterbox.fillRect(0, 0, old_w, old_h, Vec4(0,0,0,255));
    letterboxSprite.setBitmap(&letterbox);
    letterboxSprite.setZ(4999);
    clip_sprite.setZ(5001);
    shState->audio().bgm_pause();
    shState->audio().bgs_pause();
    movie->play();
    if (res_change)
      resizeScreen(old_w, old_h, true);
    shState->audio().bgm_resume();
    shState->audio().bgs_resume();
    p->fpsLimiter.disabled = limiterDisabled;
    setFrameRate(old_fps);
