...
SharedFontState::~SharedFontState()
{
  ...
  for (it = p->pool.cbegin(); it != p->pool.cend(); ++it) {
    ...
    } catch (Exception &e) { Debug() << e.msg; delete p; }
  }
  delete p;
}
...
_TTF_Font *SharedFontState::getFont(std::string family, int size)
{
  ...
  if (!loadf.empty() ) {
    ...
    if (f) {
      ...
      //Debug() << "No raw file, no pool, no font!?";
      SDL_RWclose(f);
    }
  } //else {Debug() << "Load font is empty!" << "Family isn't empty";}
  const FontSet &req = p->sets[family];
  if (req.regular.empty() && req.other.empty()) {
    family = "";//Debug() << "Load font is empty!";
  }
  key = FontKey(family, size);
  if (family.size() > 0) {//Debug() << "Third Search";
    font = p->pool.value(key);
    if (font) return font;
  }
  ...
  //Debug() << "Adding font to mana pool or something the like";
  p->pool.insert(key, font);
  return font;
}
